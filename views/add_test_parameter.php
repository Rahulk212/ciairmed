        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    <form method="post" action="<?php echo base_url(); ?>add_result/add_value_all/<?php echo $tid; ?>" id="add_parameters_ref">
                        <input type="hidden" name="test_id" value="<?php echo $tid; ?>"/>
                        <input type="hidden" name="old_test_id" value="<?php echo $event; ?>"/>
                        <input type="hidden" name="branch" value="<?= $branch_fk ?>"/>
...
?>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            ...
                                            <button class="btn btn-primary" type="button" style=" margin-top: 24px;padding: 2px 10px;" onclick="copy_value('<?php echo $tid; ?>', '<?php echo $jid; ?>');">Add Parameter</button>
                                        </div>
                                    </div>
...
<?php } ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input type="hidden" name="count_par" id="count_par" value="1">
                                                        <select class="form-control" name="par_gender_1" id="par_gender_1">
                                                            ...
                                                            <option value="N">New Born</option>
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>
                                                    <input type="text" name="group" class="form-control" style="width:50%">
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <div class="form-group">
                                        ...
                                        <textarea class="ckeditor" name="desc_test" id="description">   </textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="box-footer">
                            <input style="float:right;" class="btn btn-primary" value="Update" id="add_sub_parameter" type="button" onclick="sub_form();">
                        </div>
                    </form>
                </div>
            </div>
...
<script type="text/javascript">
    function sub_form() {
        $("#add_sub_parameter").attr("disabled", "");
    ...
    function copy_value(tid, jid) {
        var par_id = $("#exist_para").val();
        if (par_id == '') {
            $("#copy_parameter").html('Please Select Parameter.');
        } else {
            var url = "<?php echo base_url(); ?>add_result/get_value";
            $.ajax({
                type: "POST",
                url: url,
                data: {"pid": par_id, "tid": tid, "jid": jid,brnch_fk:'<?=$branch_fk;?>'}, // serializes the form's elements.
                success: function (data)
                {
                    $('#edit_div').empty();
    ...
    function edit_value(par_id,brnch) {
        if (par_id == '') {
            $("#copy_parameter").html('Please Select Parameter.');
        } else {
            var url = "<?php echo base_url(); ?>add_result/get_value";
            $.ajax({
                type: "POST",
                url: url,
                data: {"pid": par_id,brnch_fk:brnch}, // serializes the form's elements.
                success: function (data)
                {
                    $('#edit_div').empty();
                    $('#edit_div').append(data);
...
<?php if (!empty($edit_p_tid)) { ?>
        setTimeout(function () {
            edit_value('<?= $edit_p_tid; ?>','<?=$branch_fk;?>');
        }, 1000);
<?php } ?>
</script>
