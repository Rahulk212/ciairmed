...
//                }
                $total_row = $this->Logistic_test_modal->test_list_search_num($lab_fk, $search);
                $config = array();
                $config["base_url"] = base_url() . "b2b/logistic_test_master/test_list/$lab_fk?search=$search";
                ...
                $data['query'] = $this->Logistic_test_modal->test_list_search($lab_fk, $search, $config["per_page"], $page);
                $data["links"] = $this->pagination->create_links();
                $data["counts"] = $page;
				
            } else {
				
              $totalRows = $this->Logistic_test_modal->num_row('sample_test_city_price', array('status' => '1', "lab_fk" => $lab_fk));
                $config = array();
...
//            $data["query"] = $new_array;
    ...
    function test_add() {
        ...
        if (!empty($lab_fk)) {
			...
			$this->form_validation->set_rules('sprice', 'Special Price', 'trim|numeric');
			$this->form_validation->set_rules('b2bprice','B2b Price','trim|numeric');
            $this->form_validation->set_rules('desc', 'Description', 'trim');
			if ($this->form_validation->run() != FALSE) {
                $sprice = $this->input->post('sprice');
                $test=$this->input->post('test');
				$price = $this->input->post('price');
				$b2bprice = $this->input->post('b2bprice');
                $desc = $this->input->post('desc');
				
                /* $data['query'] = $this->Logistic_test_modal->master_fun_insert("sample_test_master", array("lab_fk" => $lab_fk,"description" => $desc,"test_name"=>$test,"price" =>"")); */
				$data=array("test_fk" =>$test,"lab_fk"=>$lab_fk,"description"=>$desc,"price" =>$price,"b2b_price"=>$b2bprice,"special_price"=>$sprice);
				
		
				$this->Logistic_test_modal->master_fun_insert("sample_test_city_price", array("test_fk" =>$test,"lab_fk"=>$lab_fk,"description"=>$desc,"price" =>$price,"b2b_price"=>$b2bprice,"special_price"=>$sprice));
			  ...
			  redirect("b2b/Logistic_test_master/test_list/".$lab_fk,"refresh");
                ...
                $data['test'] = $this->Logistic_test_modal->master_fun_get_tbl_val("sample_test_master",array("status" => '1'),array("test_name", "asc"));
    ...
    function test_delete() {
        ...
        $data['query'] = $this->Logistic_test_modal->master_fun_update("sample_test_city_price", array("id", $cid), array("status" => "0"));
    ...
    function test_edit() {
        ...
        //$data["city_list"] = $this->Logistic_test_modal->get_city();
        $data['query'] = $this->Logistic_test_modal->master_fun_get_tbl_val("sample_test_city_price", array("id" => $data["cid"]), array("id", "desc"));
        $this->load->library('form_validation');
        ...
        if ($this->form_validation->run() != FALSE) {
		   ...
		   $desc = $this->input->post('desc');
            ...
            $data['query'] = $this->Logistic_test_modal->master_fun_update("sample_test_city_price", array("id",$data["cid"]), array("test_fk"=>$test,"b2b_price"=>$b2bprice,"description" => $desc, "price" => $price,"special_price"=>$sprice));
...
//            }
            ...
            $this->load->view('b2b/header');
            $this->load->view('b2b/nav_logistic', $data);
            $this->load->view('b2b/testedit_views', $data);
            $this->load->view('b2b/footer');
        }
    }
    ...
    function test_export($lid) {
		...
		$query ="SELECT l.id,t.`test_name` as testname,l.price as price,l.b2b_price,l.special_price FROM sample_test_city_price l LEFT JOIN sample_test_master t ON t.`id`=l.`test_fk` WHERE l.`status`='1' and l.lab_fk=$lid";
            ...
            if (!$this->upload->do_upload('id_browes')) {
                ...
                if ($this->csvimport->get_array($file_path)) {
                    ...
                    foreach ($csv_array as $row) {
						...
						$testdetils=$this->Logistic_test_modal->test_detils($row['testname']);
						
						
						if($testdetils != ""){
							
							$testid=$testdetils->id;
						}else{
							
							$testid=$this->Logistic_test_modal->master_fun_insert("sample_test_master",array("test_name"=> $row['testname']));
						}
						
                        $old_test = $this->Logistic_test_modal->master_fun_get_tbl_val("sample_test_city_price",array("status" => '1',"test_fk" =>$testid,"lab_fk" => $lid), array("id", "desc"));
						
						$testdata=array("lab_fk" => $lid, "test_fk" =>$testid,"b2b_price"=>$row['b2b_price'],"price" => $row['price']);
						if(isset($row['special_price'])){ if($row['special_price'] != ""){ $testdata['special_price']=$row['special_price'];} }
						
                        if (empty($old_test)) {
							
                            $cnt++;
                            $this->Logistic_test_modal->master_fun_insert("sample_test_city_price",$testdata);
                        
						} else {
                         
							$cnt++;
                            $this->Logistic_test_modal->master_fun_update("sample_test_city_price",array("id",$row['id']),$testdata);
