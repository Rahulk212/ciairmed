...
class Customer_master extends CI_Controller {
    ...
    function customer_edit() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $this->session->set_flashdata("success", array("Profile successfully updated."));
            redirect("customer-master/customer-edit/" . $ccid, "refresh");
        } else {
...
<p style="color:#7e7e7e;font-size:13px;"> Your Password is: ' . $password . '  </p>
    ...
    function customer_all_details() {
        ...
        $this->load->library("Util");
        $util = new Util;
        $age = $util->get_age($data['query'][0]["dob"]);
        $data['query'][0]["age"] = $age[0];
        /* calculate age end */
        //echo "<pre>";print_r($data['query']); die(); 
        $this->load->view('header');
        $this->load->view('nav', $data);
