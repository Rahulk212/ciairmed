...
class Job_report extends CI_Controller {
    ...
    function getDailyCollectionData($start_date, $branch) {
        ...
        foreach ($branch as $b) {
            ...
            $data['userlistCashAddedBy'] = $this->job_model->get_val("SELECT   DISTINCT(job_master_receiv_amount.`added_by` ) FROM  `job_master_receiv_amount`   LEFT JOIN job_master jb     ON jb.id = job_master_receiv_amount.job_fk  WHERE ! ISNULL(job_master_receiv_amount.added_by)    AND   jb.model_type=1 and  job_master_receiv_amount.status != 0 AND DATE_FORMAT( job_master_receiv_amount.`createddate`,'%d/%m/%Y')= '" . $data['start_date'] . "' and jb.branch_fk='" . $data['branch'] . "'");
            $data['userlistAddedBy'] += $data['userlistCashAddedBy'];
            $data['branch'] = $b;
            $addedBY = array();
            foreach ($data['userlistCashAddedBy'] as $o) {
                if(!empty($o['added_by'])){
                $addedBY[] = $o['added_by'];
                }
            }
            $creditor_user = $this->job_model->get_val("SELECT  DISTINCT created_by FROM `creditors_balance` WHERE `status`='1' AND  DATE_FORMAT(`created_date`, '%Y-%m-%d') = STR_TO_DATE('" . $data['start_date'] . "','%d/%m/%Y') AND `credit`>0  AND `created_by` IN (SELECT user_fk FROM `user_branch` WHERE branch_fk='" . $data['branch'] . "' AND `status`='1')");
