...
class Service_v4 extends CI_Controller {
    ...
    function doctor_register() {
        ...
        if ($name != null & $email != null && $password != null && $mobile != null && $address != null) {
            ...
            if ($row == 0) {
                ...
                echo $this->json_data("1", "", array(array("id" => $insert, "msg" => "Submited Successfully")));
            } else {
    ...
    function forget_password() {
        ...
        if ($email != NULL) {
            ...
            if (!empty($row)) {
                ...
                echo $this->json_data("0", "Invalid Email", "");
            }
        } else {
...
//$this->email->to("jeel@virtualheight.com");
  ...
  WHERE `package_master`.`status` = '1' 
    ...
    function add_job() {
        ...
        if ($user_fk != "" && $testfk != "") {
            $insert = $this->service_model->master_fun_insert("job_master", array("cust_fk" => $user_fk, "date" => $date, "order_id" => $order_id, "doctor" => $doctor, "other_reference" => $other_reference, "payment_type" => "Wallet", "portal" => $type, "booking_info" => $booking_fk));
            $test = explode(',', $testfk);
            $price = 0;
            ...
            if ($update) {
                ...
                if ($total >= $price) {
                    ...
                    $this->service_model->master_fun_update("job_master", $insert, array("price" => $amount, "payable_amount" => 0));
                    $j_id = $insert;
                } else {
                    $query = $this->service_model->master_fun_get_tbl_val("offer_master", array("status" => 1), array("id", "desc"));
                    $caseback_per = $query[0]['caseback_per'];
                    ...
                    $query = $this->service_model->master_fun_get_tbl_val("wallet_master", array("status" => 1, "cust_fk" => $user_fk), array("id", "desc"));
                    $total1 = $query[0]['total'];
                    $this->service_model->master_fun_update("job_master", $insert, array("price" => $amount, "payable_amount" => $price - $total));
                    $j_id = $insert;
                }
                $query = $this->service_model->master_fun_get_tbl_val("wallet_master", array("status" => 1, "cust_fk" => $user_fk), array("id", "desc"));
...
/// ---------for half payment add_job --
    ...
    function package_list() {
        ...
        /* Nishit code start */
        $data = $this->service_model->master_fun_get_tbl_val("package_master", array("status" => 1), array("order", "asc"), array("id", "title", "image,desc_app"));
        $cnt = 0;
        foreach ($data as $key) {
            if ($city != '') {
    ...
    function health_feed() {
        ...
        /* Check api version */
        $check_version_array = array("android" => array("version" => "43", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "2", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
...
' . $payfrom_wallet . '
    ...
    function assign_phlebo_job($job_id) {
        ...
        $phlebo_list = $this->service_model->master_fun_get_tbl_val("phlebo_master", array('status' => "1"), array("id", "asc"));
    ...
    function get_job_id() {
        $get_max_id = $this->service_model->get_val("SELECT order_id AS `max` FROM job_master ORDER BY id DESC LIMIT 0,1");
        $new_id = $get_max_id[0]['max'] + 1;
        $i = 0;
        $cnt = 0;
        while ($i == 0) {
            $new_id = $new_id + $cnt;
            $check_number = $this->service_model->get_val("SELECT count(order_id) AS `count` FROM job_master where order_id='" . $new_id . "'");
