...
class Service_v7 extends CI_Controller {
...
' . $payfrom_wallet . '
    ...
    function home_page() {
        ...
        /* Check api version */
        $check_version_array = array("android" => array("version" => "47", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "1.3.7", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
        $configuration = array("MERCHANT_ID" => $MERCHANT_ID, "MERCHANT" => $MERCHANT_KEY, "SALT" => $SALT, 'URl' => $URL, "WalletIsUsedInPackagePayment" => "0");
        $data3 = $this->service_model->master_fun_get_tbl_val("offer_master", array("status" => 1), array("id", "asc"));
    ...
    function get_fb_user_info() {
        ...
        $data = $this->service_model->get_val("SELECT email FROM `customer_master` WHERE fbid='" . $fb_id . "'");
        if (!empty($data) && !empty($fb_id)) {
            echo $this->json_data("1", "", $data);
        } else {
            echo $this->json_data("0", "Data not available.", "");
        }
