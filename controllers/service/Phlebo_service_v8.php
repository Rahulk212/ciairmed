...
class Phlebo_service_v8 extends CI_Controller {
...
FROM
  ...
  LEFT JOIN `phlebo_assign_job` 
    ...
    LEFT JOIN `phlebo_time_slot` 
      ON `phlebo_time_slot`.`id` = `phlebo_assign_job`.`time_fk` 
WHERE job_master.status IN (6, 1) AND `job_master`.`date`<='" . date("Y-m-d") . " 23:59:59' AND `job_master`.`date`>='" . $last_date . " 00:00:00' AND `phlebo_assign_job`.phlebo_fk='" . $pid . "' AND `phlebo_assign_job`.`is_accept`='0'
ORDER BY `job_master`.`id` DESC ");
        if (!empty($job_list)) {
            $data = array();
            foreach ($job_list as $key) {
...
ORDER BY `logistic_log`.`id` DESC ");
    ...
    function collect_from() {
        $pid = $this->input->get_post('phlebo_fk');
        $p_city = $this->service_model->master_fun_get_tbl_val('phlebo_master', array("id" => $pid), array("id", "asc"));
        if (!empty($p_city)) {
            $data = $this->service_model->master_fun_get_tbl_val('collect_from', array("status" => 1, "city" => $p_city[0]["test_city"]), array("name", "asc"));
        } else {
...
WHERE `doctor_master`.`status` = '1' AND `doctor_master`.`city` = '" . $data1[0]["city_fk"] . "'
  AND (
    ...
    function accep_assign_phlebo_job() {
        ...
        $insert = $this->service_model->master_fun_update("phlebo_assign_job", array("job_fk", $job_id), $data);
        /* Nishit code end */
    ...
    function check_api_version() {
        $check_version_array = array("android" => array("version" => "19", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "2", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
