...
//                }
			...
			$barcodeall=explode(",",$barcode);
				  if($barcodeall[0] != ""){
				foreach($barcodeall as $bar){ $this->service_model->master_fun_insert("phlebo_barcode", array("job_fk"=>$insert,"barcode"=>$bar)); }
			     }
...
WHERE `doctor_master`.`status` = '1' AND `doctor_master`.`city` = '" . $data1[0]["city_fk"] . "'
  AND (
    ...
    function check_api_version() {
        $check_version_array = array("android" => array("version" => "20", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "2", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
