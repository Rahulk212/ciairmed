...
//////////vishal code start 09-02-2016////////
    function static_name() {
        echo $this->json_data("1", "", array(array("name" => "This is test", "key" => "2")));
    }
...
//$this->email->to("jeel@virtualheight.com");
    ...
    function doctor_view_report() {
        ...
        if ($jid != null) {
            ...
            foreach ($tests as $key) {
                ...
                $report_data = $this->service_model->get_result("SELECT * from report_master where job_fk='" . $jid . "' and status='1'");
                $report = '';
                $original = '';
                $description = "";
  ...
  WHERE `package_master`.`status` = '1' 
    ...
    function doctor_stat() {
        ...
        if ($user_fk != null) {
            $data = $this->doctor_app_model->doctor_stat1($user_fk, $month, $year);
            if (empty($data)) {
                echo $this->json_data("0", "no data available", "");
            } else {
                echo $this->json_data("1", "", $data);
