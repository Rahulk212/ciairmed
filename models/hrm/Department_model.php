...
Class Department_model extends CI_Model {
    ...
    function search_num($search) {
        ...
        $query = $this->db->query("SELECT * FROM `hrm_department` where status ='1' $temp ");
    ...
    function list_search($search, $one, $two) {
        ...
        $query = $this->db->query("SELECT * FROM `hrm_department` where status ='1' $temp  ORDER BY id desc LIMIT $two,$one");
        return $query->result();
    }
