...
AND `test_cities`.`status`='1'");
	...
	function admin_list($one, $two) {

        $query = $this->db->query("SELECT a.*,c.name as cityname FROM `admin_master` a left join test_cities c on c.id=a.city_fk WHERE a.status='1'  ORDER BY a.id DESC LIMIT $two,$one ");
