...
AND `test_cities`.`status`='1'");
    ...
    function autho_list($one, $two,$branch) {
        if(!empty($branch)) {
            $query = $this->db->query("SELECT i.test_date_time,i.barcode,i.id,c.full_name,j.branch_fk FROM `instument_data_storage_new` i join job_master j on j.barcode = i.barcode join customer_master c on c.id = j.cust_fk WHERE i.`status`='1' and j.branch_fk in ($branch) group by i.barcode ORDER BY i.id DESC LIMIT $two,$one ");
        } else {
            $query = $this->db->query("SELECT i.test_date_time,i.barcode,i.id,c.full_name,j.branch_fk FROM `instument_data_storage_new` i join job_master j on j.barcode = i.barcode join customer_master c on c.id = j.cust_fk WHERE i.`status`='1' group by i.barcode ORDER BY i.id DESC LIMIT $two,$one ");
        }
    ...
    function autho_list_count($branch) {
        if(!empty($branch)) {
            $query = $this->db->query("SELECT i.test_date_time,i.barcode,i.id,c.full_name,j.branch_fk FROM `instument_data_storage_new` i join job_master j on j.barcode = i.barcode join customer_master c on c.id = j.cust_fk WHERE i.`status`='1' and j.branch_fk in ($branch) group by i.barcode ORDER BY i.id DESC");
        } else {
            $query = $this->db->query("SELECT i.test_date_time,i.barcode,i.id,c.full_name,j.branch_fk FROM `instument_data_storage_new` i join job_master j on j.barcode = i.barcode join customer_master c on c.id = j.cust_fk WHERE i.`status`='1' group by i.barcode ORDER BY i.id DESC");
        }
